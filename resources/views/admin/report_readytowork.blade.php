...
<head>
    ...
    <title>Onstru | Ready To Work Report</title>
        ...
        <div class="body-main">
            ...
            <div class="main-div px-4 py-1">
                <div class="body-head mb-3">
                    <h4 class="m-0">Ready To Work Report</h4>
                </div>
                        ...
                        <div class="container-fluid listtable mt-2">
                            ...
                            <div class="table-wrapper">
                                <table class="table" id="listingTable">
                                    ...
                                    <tbody>
                                        @foreach ($readytowork as $ready)
                                            <tr id="{{ $ready->id }}">
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $ready->user->name ?? '-' }}</td>
                                                <td>{{ $ready->user->user_name ?? '-'}}</td>
                                                <td>{{ $ready->user->as_a ?? 'Consumer' }}</td>
                                                <td>{{ $ready->days ?? '-' }} Days</td>
                                                <td>₹ {{ $ready->amount * $ready->days ?? '-' }}</td>
                                                <td>
                                                    <div class="d-flex align-items-center column-gap-2">
                                                        <a href="{{ url('user_detail', ['id' => $ready->created_by]) }}"
