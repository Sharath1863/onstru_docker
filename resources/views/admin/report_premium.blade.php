...
<head>
    ...
    <title>Onstru | Premium Report</title>
...
<body>
        ...
        <div class="body-main">
            ...
            <div class="main-div px-4 py-1">
                <div class="body-head mb-3">
                    <h4 class="m-0">Premium Report</h4>
                </div>
                        ...
                        <div class="container-fluid listtable mt-2">
                            <div class="filter-container">
                                <div class="filter-container-start">
                                    <select class="form-select filter-option" id="userDropdown">
                            ...
                            <div class="table-wrapper">
                                <table class="table" id="userTable">
                                    ...
                                    <tbody>
                                        @foreach ($premiumReport as $premium)
                                            <tr id="{{ $premium->id }}">
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $premium->user->name ?? '-' }}</td>
                                                <td>{{ $premium->user->user_name ?? '-'}}</td>
                                                <td>{{ $premium->user->as_a ?? 'Consumer'}}</td>
                                                <td>{{ $premium->user->number ?? '-' }}</td>
                                                <td>₹ {{ $premium->price ?? '-' }}</td>
                                                <td>
                                                    <div class="d-flex align-items-center column-gap-2">
                                                        <a href="{{ url('user_detail', ['id' => $premium->user_id]) }}"
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            initTable('#userTable', '#userDropdown', '#userInput');
        });
    </script>
