...
<head>
    ...
    <title>Onstru | Products Report</title>
...
<body>
        ...
        <div class="body-main">
            ...
            <div class="main-div px-4 py-1">
                <div class="body-head mb-3">
                    <h4 class="m-0">Products Report</h4>
                </div>

                <div class="profile-tabs">
                    <ul class="nav nav-tabs d-flex align-items-center gap-md-3 gap-sm-2 flex-row border-0"
                ...
                <div class="tab-content" id="myTabContent">
                        ...
                        <div class="container-fluid listtable mt-2">
                            ...
                            <div class="table-wrapper">
                                <table class="table" id="listingTable">
                                    ...
                                    <tbody>
                                        @foreach ($productListing as $products)
                                            @if ($products->type == 'list')
                                                <tr id="{{ $products->id }}">
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $products->product->name ?? '-' }}</td>
                                                    <td>{{ $products->product->locationRelation->value ?? '-' }}</td>
                                                    <td>{{ $products->product->vendor->name ?? '-' }}</td>
                                                    <td>₹ {{ $products->amount ?? '-' }}</td>
                                                    <td>
                                                        <div class="d-flex align-items-center column-gap-2">
                                                            <a href="{{ route('product_detail', ['id' => $products->product_id]) }}"
                                                                data-bs-toggle="tooltip" data-bs-title="View Product">
                        ...
                        <div class="container-fluid listtable mt-2">
                            <div class="filter-container">
                                <div class="filter-container-start">
                                    ...
                                    <input type="text" class="form-control" id="highlightInput"
                                        placeholder=" Search">
                                </div>
                            </div>
                            <div class="table-wrapper">
                    ...
                    <div class="tab-pane fade" id="margin" role="tabpanel">
                        <div class="container-fluid listtable mt-2">
                            <div class="filter-container">
                                <div class="filter-container-start">
                                    <select class="form-select filter-option" id="marginDropdown">
                                        <option value="All" selected>All</option>
                                    </select>
                                    <input type="text" class="form-control" id="marginInput" placeholder=" Search">
                                </div>
                            </div>
                            <div class="table-wrapper">
                                <table class="table" id="marginTable">
                                    ...
                                    <tbody>
                                        @foreach ($productincome as $products)
                                            <tr id="{{ $products->id }}">
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $products->product->name ?? '-' }}</td>
                                                <td>{{ $products->product->locationRelation->value ?? '-' }}</td>
                                                <td>{{ $products->product->vendor->name ?? '-' }}</td>
                                                <td>₹ {{ $products->margin ?? '0' }}</td>
                                                <td>
                                                    <div class="d-flex align-items-center column-gap-2">
                                                        <a href="{{ route('product_detail', ['id' => $products->product_id]) }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            initTable('#marginTable', '#marginDropdown', '#marginInput');
        });
    </script>
