...
@section('content')
    ...
    <div class="container main-div">
        ...
        <div class="form-div py-2 px-3 mt-4" id="job_post">
            <form action="{{ route('jobs.store') }}" method="POST" id="jobAdd">
                @csrf
                <div class="row">
                    <input type="hidden" name="created_by" value="{{ Auth::id() }}">
                    <div class="col-sm-12 col-md-4 mb-3">
                        <label for="jobtitle">Job Title <span>*</span></label>
                        <input type="text" name="title" id="jobtitle" class="form-control" value="{{ old('title') }}"
                            required autofocus>
                    </div>

                    <div class="col-sm-12 col-md-4 mb-3">
                        <label for="skills">Required Skills</label>
                        <textarea rows="1" name="skills" id="skills" class="form-control">{{ old('skills') }}</textarea>
                    </div>

                    <div class="col-sm-12 col-md-4 mb-3">
                        <label for="description">Job Description <span>*</span></label>
                        <textarea rows="1" name="description" id="description" class="form-control" required>{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <select name="qualification" id="qualification" class="form-select" required>
                            ...
                            <option value="BE_Civil" {{ old('qualification') == 'BE_Civil' ? 'selected' : '' }}>B.E.
                                (Civil)
                            </option>
                            <option value="BE" {{ old('qualification') == 'BE' ? 'selected' : '' }}>B.E. (Bachelor of
                                Engineering)</option>
                            <option value="BTech" {{ old('qualification') == 'BTech' ? 'selected' : '' }}>B.Tech (Bachelor
                                of
                                Technology)</option>
                            <option value="BCA" {{ old('qualification') == 'BCA' ? 'selected' : '' }}>BCA (Bachelor of
                                Computer Application)</option>
                            <option value="BSC" {{ old('qualification') == 'BSC' ? 'selected' : '' }}>B.Sc</option>
                            <option value="BCom" {{ old('qualification') == 'BCom' ? 'selected' : '' }}>B.Com</option>
                            <option value="BA" {{ old('qualification') == 'BA' ? 'selected' : '' }}>BA</option>
                            <option value="BBA" {{ old('qualification') == 'BBA' ? 'selected' : '' }}>BBA</option>
                            <option value="ME" {{ old('qualification') == 'ME' ? 'selected' : '' }}>M.E.</option>
                            <option value="MTech" {{ old('qualification') == 'MTech' ? 'selected' : '' }}>M.Tech</option>
                            <option value="MCA" {{ old('qualification') == 'MCA' ? 'selected' : '' }}>MCA</option>
                            <option value="MSC" {{ old('qualification') == 'MSC' ? 'selected' : '' }}>M.Sc</option>
                            <option value="MCom" {{ old('qualification') == 'MCom' ? 'selected' : '' }}>M.Com</option>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <select name="job_type" id="job_type" class="form-select" required>
                            <option value="{{ old('job_type') }}" disabled {{ old('job_type') ? '' : 'selected' }}>Select
                                Job
                                Type</option>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <select name="category" id="category" class="form-select" required>
                            <option value="" disabled {{ old('category') ? '' : 'selected' }}>Select Job Category
                            </option>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <input type="number" name="salary" id="salary" class="form-control"
                            value="{{ old('salary') }}" required min="0" step="1"
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <input type="number" name="no_of_openings" id="no_of_openings" class="form-control"
                            value="{{ old('no_of_openings') }}" required min="1" step="1"
                            oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                    </div>

                    <div class="col-sm-12 col-md-4 mb-3">
                        <label for="location">Location <span>*</span></label>
                        <select name="location" class="form-select" value="" id="location" required>
                    ...
                    <div class="col-sm-12 col-md-4 mb-3">
                        ...
                        <textarea rows="1" name="benefits" id="benefits" class="form-control">{{ old('benefits') }}</textarea>
                    </div>

                    <div
                        class="col-sm-12 col-md-12 d-flex align-items-center justify-content-sm-start justify-content-sm-end mt-3">
                        <button type="submit" class="formbtn">Add Job</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAmDotOMSahWwA7WAjmtremPvNNRN1Nye0&libraries=places">
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('jobAdd');
