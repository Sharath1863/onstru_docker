...
@if (count($posts) == 0)
    <div class="side-cards shadow-none border-0">
        <div class="cards-content d-flex align-items-center justify-content-center flex-column gap-2">
            <img src="{{ asset('assets/images/Empty/NoPosts.png') }}" height="200px" class="d-flex mx-auto mb-2"
                alt="">
    ...
    <div class="post-cards position-relative">
        @foreach ($posts as $post)
            ...
            @if (is_array($files) && count($files) > 0)
                ...
                <div class="product-card post-bg position-relative rounded-2 ind_post post_{{ $post->id }}"
                    data-post-id="{{ $post->id }}" data-bs-toggle="modal" data-bs-target="#ind_post"
                    data-type="{{ $type }}" data-share-title="{{ $post->name ?? 'New Post' }}"
                    data-share-url="{{ $fileUrls[0] ?? '' }}" data-share-text="{{ $fileText }}"
                    data-assets="{{ json_encode($fileUrls) }}" data-post-like="{{ $post->like_cnt }}"
                    data-post-con="{{ $post->created_at->format('M d') }}" data-is_like="{{ $post->is_liked }}"
                    data-is_save="{{ $post->is_saved }}" data-caption="{{ $post->caption ?? '' }}"
                        ...
                        @if ($post->sense == 1)
                            ...
                            <div class="sensitive-overlay z-3">
                                <div class="overlay-content text-center">
                                    <img src="{{ asset('assets/images/Sensitive.png') }}" height="50px"
            ...
            @else
                <div class="side-cards shadow-none border-0">
                    <div class="cards-content d-flex align-items-center justify-content-center flex-column gap-2">
                        <img src="{{ asset('assets/images/Empty/NoPosts.png') }}" width="100%" class="d-flex m-auto"
                            alt="">
                    </div>
                </div>
            @endif
